...
def test_publish_update_messages_for_airac_includes_provider(mock_publish):
    airac_data = AiracData(airac="250417", provider="faa_tpp")
    ...
    assert message_attributes["provider"]["StringValue"] == "faa_tpp"


@patch("aviationapi.lib.messengers.trigger_chart_post_processor.TOPIC.publish")
def test_publish_success_message_includes_provider(mock_publish):
